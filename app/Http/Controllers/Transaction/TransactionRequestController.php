...
class TransactionRequestController extends Controller
{
    ...
    public function loaddata(){
        ...
        $this->userlog($notes,$status);
            ...
            return view('transaction.requests.index',compact('RenterRequests'))
    ...
    public function storedata($request){
        ...
        if ($RenterRequests) {
            ...
            return redirect()->route('transactionrequest.create')
    ...
    public function updatedata($request,$salerid){
        ...
        if($RenterRequests->totalsales == 0){
            ...
            return redirect()->route('transactionrequest.index')
    ...
    public function displayall()
    {
        ...
        return view('dashboardoverview.index',['RenterRequests' => $RenterRequests]);
    }

    public function selectbranch(){
        $branches = branch::query()->paginate(5);;
        return view('transaction.requests.select-branch',['branches' => $branches])
                        ->with('i', (request()->input('page', 1) - 1) * 5);
    }
    public function selectcabinet(Request $request,$branch){
        ...
        return view('transaction.requests.select-cabinet',['cabinet' => $cabinet])->with(['branchname'=> $branches->branchname]);
    }
    ...
    public function create($RenterRequests)
    {
        ...
        if(auth()->user()->status =='Active'){
            if(auth()->user()->accesstype =='Cashier'){
                ...
                return view('transaction.requests.create',['cabinet' => $cabinet])
                                        ->with(['branch' => $branch]);
            }elseif(auth()->user()->accesstype =='Administrator'){
                return view('transaction.requests.create',['cabinet' => $cabinet])
                                        ->with(['branch' => $branch]);
    ...
    public function edit($salerid)
    {
        ...
        if(auth()->user()->status =='Active'){
            if(auth()->user()->accesstype =='Cashier'){
                return view('transaction.requests.edit')
